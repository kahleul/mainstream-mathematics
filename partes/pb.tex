...
\begin{prop}
    Sejam $(\Omega, \mathcal{F}, \Pb)$ um espaço de probabilidade e $B \in \mathcal{F}$. A terna $(\Omega, \mathcal{F}, \bar{\Pb})$, onde $\bar{\Pb} : \mathcal{F} \to \R$ é uma função definida por $\bar{\Pb}(A) := \Pb(A|B)$ para todo $A \in \mathcal{F}$, é um espaço de probabilidade.
\end{prop}
