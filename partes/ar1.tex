...
\begin{defi}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            ...
            \item Seja $P : a = x_0 < x_1 < \cdots < x_n = b$ uma partição de um intervalo $[a,b]$. A \textit{amplitude} do $i$-ésimo intervalo $[x_{i-1}, x_i]$, para cada $i \in [n]$, é definida como $\Delta x_i := x_i - x_{i-1}$.
            \item Uma partição $Q$ de um intervalo $[a,b]$ é um \textit{refinamento} de uma partição $P$ de $[a,b]$ se $P \subseteq Q$.
        \end{enumerate}
\end{defi}
